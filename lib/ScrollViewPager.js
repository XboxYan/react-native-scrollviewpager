...
export default class ScrollViewPager extends PureComponent {
    ...
    render() {
        ...
        const {tabBarOptions:{wrapstyle,tabconStyle,style,labelStyle,tabStyle,indicatorStyle,activeTintColor}} = this.props;
            ...
            <View style={[styles.container,wrapstyle]}>
                <View style={[styles.scrolltabbar,style]}>
                    <ScrollView
                        ...
                        contentContainerStyle={[tabconStyle,swipeEnabled&&{justifyContent:'flex-start'}]}
                    ...
                    >
                        {
                            tablabel.map((item,i)=>(
                                <Touchable onLayout={(e)=>this.onlayout(e,i)} key={i} onPress={()=>{this.onSetPage(i)}} style={[styles.tabbaritem,tabStyle]}><Text numberOfLines={2} style={[styles.tabbartext,labelStyle,(pageIndex===i)&&{color:activeTintColor}]}>{item}</Text></Touchable>
                            ))
                        }
                        <View style={[styles.tabline, { width:this.tabswidth[pageIndex]||initialWidth,left: this.tabsdir[pageIndex] }]}>
